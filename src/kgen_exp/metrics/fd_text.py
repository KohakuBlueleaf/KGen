...
class FDTextRunner(TextMetricRunner):
    ...
    def frechet_distance(self):
        ...
        cov_a_num = a_features_cov_sum - len(a_features) * torch.matmul(mean_a.T, mean_a)
        cov_b_num = b_features_cov_sum - len(b_features) * torch.matmul(mean_b.T, mean_b)
        cov_a = cov_a_num / (len(a_features) - 1)
        cov_b = cov_b_num / (len(b_features) - 1)
        return self.compute_frechet_distance(mean_a.squeeze(0), cov_a, mean_b.squeeze(0), cov_b)
