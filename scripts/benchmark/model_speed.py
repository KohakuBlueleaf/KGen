...
def task(tags: list[str], nl_prompt: str):
    ...
    meta, operations, general, nl_prompt = tipo.parse_tipo_request(
        ...
        or (
            not nl_prompt
            and ("<|generated|>" in DEFAULT_FORMAT or "<|extended|>" in DEFAULT_FORMAT)
        ),
    )
    meta["aspect_ratio"] = f"{width / height:.1f}"
    result, timing = tipo.tipo_runner(
        meta, operations, general, nl_prompt, retry_criteria=lambda *args: True
    )
...
if __name__ == "__main__":
    ...
    for model_name, gguf_list in models.tipo_model_list:
