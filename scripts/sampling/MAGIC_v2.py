...
class MCTSNode(SampleNode):
    ...
    def select(self, exploration=1.4) -> "MCTSNode":
        ...
        while node.childs and any(child.active for child in node.childs):
            ...
            if not active_childs:
                node.spent
                return self.select(exploration)
    ...
    def expand(self, splitters=None, ids_splitters=None) -> tuple["MCTSNode", int] | tuple[None, int]:
        ...
        # determine whether node is promising
        score_threshold = self.score / self.visits if self.visits > 0 else float('-inf')
        parent_score = self.parent.score / self.parent.visits if self.parent else 0
        
        if score_threshold > parent_score:
            num_childs = max(2, 5 - self.depth)
        else:
            num_childs = max(1, 3 - self.depth)
...
def MAGIC_sample(
    ...
    while len(results) < variations:
        ...
        print(f"Select depth: {node.depth}")
        ...
        if node and not any(result == node.simulated_result for result in results): 
            # NOTE: the dupe check is necessary
            # since sometimes this method gets collisions
            # maybe exploration can fix that but doesn't seem scalable indefinitely
